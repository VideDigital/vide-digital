...
| Painel de dados do cliente na Central de Atendimento | Mostra nome/canal/setor/responsável/notas internas/tags já gravados na própria conversa | Cruzar com `leads`/`pedidos` (histórico de pedidos, contagem de conversas) sem duplicar dado |
...
1. **Deploy das Rules deste ciclo**: as Rules de `chats`/`mensagens`/`templates` reforçadas neste ciclo (Central de Atendimento) só valem depois de o workflow de deploy publicar `firestore.rules` em produção — até lá, produção continua na versão anterior.
...
1. Categorias e ações dedicadas (duplicar/arquivar) para templates de atendimento, sem quebrar os templates de automação de leads que já usam a mesma coleção.
2. Cruzar o painel de dados do cliente da Central de Atendimento com `leads`/`pedidos` (histórico real, sem duplicar dado nem criar um segundo perfil de cliente).
