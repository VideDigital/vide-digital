...
| IA de Negócio pública (`askPublicBusinessAI`, visitante da loja) — código completo, testado e revisado; deploy em andamento | Segunda metade do pedido original (widget na `loja.html`, sem login, contexto restrito a produtos ativos, toggle `iaNegocioPublicaAtiva` no dashboard gated por "central-ia"). Falta: rodar o deploy da Function (junto de `askBusinessAI`, mesmo workflow) e o deploy das Rules novas (workflow "Deploy Firebase Spark", separado) — só depois dos dois uma conversa real de visitante pode ser testada. Ver "IA de Negócio pública" em `docs/IA_NEGOCIO.md`. | Rodar os dois deploys (Functions + Spark/Rules) e testar uma conversa real de visitante numa loja Pro+ com o toggle ativado |
...
3. ~~**IA real de negócio (`askBusinessAI`)**~~ — **resolvido neste ciclo**: `askBusinessAI` publicada com sucesso em produção via `.github/workflows/firebase-deploy-functions.yml`, com resposta real confirmada do Gemini. Duas concessões de IAM feitas manualmente uma única vez (documentadas em `docs/IA_NEGOCIO.md`): "Secret Manager Secret Accessor" pra conta de deploy e pra conta de execução padrão das Functions (`891590456336-compute@developer.gserviceaccount.com`). `askPublicBusinessAI` (visitante da loja pública) foi construída na sequência, reusando a mesma chave/secret — deploy pendente, mesmo workflow.
4. **WhatsApp oficial**: depende do backend externo, nada implementado ainda.

## Próximas prioridades reais

1. **IA de Negócio pública: rodar os dois deploys pendentes** (Functions — `askPublicBusinessAI` junto de `askBusinessAI` — e Rules, workflow "Deploy Firebase Spark") e testar uma conversa real de visitante numa loja Pro+ com o toggle ativado (ver `docs/IA_NEGOCIO.md`).
