...
3. **IA real de negócio (`askBusinessAI`)**: chave do Gemini criada e configurada como secret (`GEMINI_API_KEY`, Secret Manager, projeto `vide-digital-saas`) — falta só rodar o workflow de deploy dedicado (`.github/workflows/firebase-deploy-functions.yml`), com confirmação explícita do usuário. Ver "O que falta pra isso funcionar em produção" em `docs/IA_NEGOCIO.md`.
4. **WhatsApp oficial**: depende do backend externo, nada implementado ainda.

## Próximas prioridades reais

1. **IA de Negócio: rodar o workflow de deploy dedicado e publicar `askBusinessAI`** — chave e secret já configurados pelo usuário, workflow já criado; falta só a confirmação explícita pra rodar (ver `docs/IA_NEGOCIO.md`).
