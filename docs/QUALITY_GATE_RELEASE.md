...
O job `ui-login` é bloqueante desde 2026-07-28 (não tem mais `continue-on-error: true`) — os 3 scripts foram confirmados verdes de ponta a ponta em CI real, ver "Limitações reais" abaixo e `docs/HANDOFF_2026-07-28.md`.
...
- **`test:ui:login` / `test:ui:flows` / `test:ui:responsive` foram confirmados 100% verdes num run real de GitHub Actions em 2026-07-28** (run completo, os 3 steps com `"conclusion":"success"` individualmente, não só o job por fora) — ver `docs/HANDOFF_2026-07-28.md` pro run ID, SHA e histórico completo de bugs reais encontrados e corrigidos ao longo do processo. O job `ui-login` deixou de ter `continue-on-error: true` a partir desse commit: uma falha real nessa suíte agora bloqueia o merge, como qualquer outro job do gate.
- **O sandbox de desenvolvimento usado para escrever/editar este projeto continua bloqueando `www.gstatic.com`** (CDN do SDK do Firebase) por política de egress — isso não mudou e não muda a validação em CI, mas significa que `test:ui:login`/`test:ui:flows`/`test:ui:responsive` continuam impossíveis de rodar localmente nesse sandbox específico. Rode-os num ambiente com egress real (qualquer runner GitHub Actions padrão, ou uma máquina de desenvolvimento sem proxy corporativo bloqueando CDNs do Google) quando precisar depurar localmente antes de confiar no resultado.
