...
- **Nada é duplicado de forma permanente**: `produtoIds` é a única referência persistida; nome/preço são lidos do catálogo (`produtos`, filtrado por `criadoPor == storeUid`, `limit(300)` como teto de leitura) toda vez que o item é aberto para edição ou salvo — se o preço mudar no catálogo, o próximo salvamento do item de conhecimento já reflete o valor novo. Busca é por substring em memória sobre o catálogo carregado (Firestore não tem "contains"); um tenant com catálogo acima de 300 produtos ativos não veria os excedentes no buscador — considerado aceitável hoje (nenhum plano ativo se aproxima disso), registrado para reavaliar se mudar.
