...
CI review also identified missing order-to-lead fields. The complete current orders-engine payload is now exercised by an Emulator test extracted from its writer, and admitted by the allowlist. Existing literal dotted keys are preserved for compatibility; nested-field format correction is a separate investigation.
