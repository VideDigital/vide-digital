...
Selection removal belongs only to the bulk caller; auxiliary writes preserve the user's selection. Automation error recovery reloads server state without automatically repeating rejected writes. A later explicit reload may retry. Merge enforces terminal stage probabilities (won 100%, lost 0%, automatic) even when stage and probability originate from different sparse legacy records. Legacy `utm_*` fields are resolved into canonical UTM fields during merge, respecting canonical presence and explicit clears; new legacy aliases are never written.
